...
impl DemoArgs {
    #[instrument(name = "Executing demo command")]
...
async fn install_cmd(
    ...
    // let mut pb = ProgressOutput::new(4, 15, "Installing demo".into());
    // let task = pb.add_subtask("Building stack list", "", 6).await;
    ...
    // task.send_line("Building release list").await;
    ...
    // task.send_line("Installing local cluster").await;

    // Install local cluster if needed
    args.local_cluster.install_if_needed(None).await?;

    // task.send_line("Installing stack").await;

    // Install the stack
    stack_spec.install(release_list, &common_args.operator_namespace)?;

    // task.send_line("Installing stack manifests").await;
    ...
    // task.send_line("Installing demo manifests").await;
